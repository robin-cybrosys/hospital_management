...
class HospitalConsultation(models.Model):
    ...
    status = fields.Selection(
        [('draft_state', 'Draft'), ('confirm_state', 'Confirmed')], default="draft_state", copy=False)
    ...
    def consult_btn(self):
        ...
        self.consultation_id = self.env['ir.sequence'].next_by_code(
            'hospital.consult')
        # rec = self.env['hospital.out_patient'].search([
        #     ('status', '!=', 'draft_state'),
        #     ('patient_card_id', '=', self.patient_card_id.id)])
        # print("aa", rec.patient_card_id, self.patient_card_id.id)
        # if rec.patient_card_id.id == self.patient_card_id.id:
        #     self.write({'consultation_id': (rec.token_no.replace(
        #         "CONS/", "CONS/OP/")) + " | " + rec.patient_id.name})
    ...
    #     vals['consultation_id'] = self.env['ir.sequence'].next_by_code(
    #         'hospital.consult')
